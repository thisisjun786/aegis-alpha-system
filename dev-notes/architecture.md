...
거부는 그대로 두고, 관측 연구 경로가 그 값을 `feature_values`에 binary64로 반올림 없이
보존한다. 이 계약은 price_role을 reference로, certified를 false로 고정하고 값 범위를
...
조상의 변환까지 같은 정의인지 확인한 뒤에만 게시된다. 연구 수익률 프록시와 관측 가격은
