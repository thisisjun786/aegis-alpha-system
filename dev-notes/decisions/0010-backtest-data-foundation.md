...
2026-09-08: 제품 책임은 [0015](0015-research-engine-product-boundary.md)를 따른다. 데이터 의미·
출처·시점·자격 요구는 유지하며 아래의 세 모듈은 현행 계약이다. 모든 외부 계산 호출이
세 모듈을 요구한다는 뜻은 아니다.
...
현재 내장 DB 설치·전략 등록·typed generation 게시·cutoff 조회는 native CLI에 연결돼 있다.
기존 PostgreSQL 데이터 채택과 Parquet 가격 조회는 `legacy-db`·`legacy-data` 경로다.
내장 시장 reader의 revision 가시성 조회와 전체 실행 bundle·전략 실행의 통합 완성은 구분한다.
