...
2026-09-08: 제품 목표는 [0015](0015-research-engine-product-boundary.md)를 따른다. 이 결정의
설치·보존 계약은 유지한다. 로컬 소켓·상시 앱·예약 실행은 승인된 설계이지만 현재 CLI에
구현되지 않았다. 아래 설계와 [현재 명령](../operations.md)을 구분한다.
...
~/.aas/
  ...
  runtime/             # 실행 상태 공간; 로컬 소켓은 미구현 설계
```
...
현재 `storage/workspace.py`는 일회 CLI가 설치·파일 잠금을 보유한 동안 DB 연결을 열고
명령이 끝나면 닫는다. 잠금 경쟁은 `installation_busy`로 실패한다. 아래의 상시 앱·소켓·
호스트 실행기 흐름은 승인된 후속 설계이며 현재 구현의 실행 절차가 아니다.
...
2. `aas doctor`: 저장소 정체성·schema를 검사하고 루트·DB 위치·등록 수를 표시한다.
   현재 출력은 예약 실행과 공급자 라이브 검증이 시작되지 않았음을 명시한다.
   모든 공급자 권한 상태를 종합 진단하는 명령은 아니다.
...
현재 백업은 설치 잠금을 잡고 pending 작업이나 실행 중 분석이 있으면 거부한다.
다음은 상시 앱 연결 이후에도 지켜야 할 유지보수 계약이다. 백업은 같은 앱에 유지보수를 요청한다. 새 쓰기를 막고 수집·전략 등록·분석 연결과
...
자동 schema 업그레이드·다운그레이드는 현재 제공하지 않는다. 후속 업데이트 설계는
잠금·백업 후 schema별 migration을 수행하고 일부만 성공하면 기동을 차단해야 한다.
...
현재 연결된 로컬 기능과 미구현 과제는 [전환 표](../design/backtest-data-foundation.md#전환-계획과-기존-코드)로
구분한다. 설계 채택만으로 실제 데이터 이전·서비스 재시작·배포 성공을 주장하지 않는다.
