...
그래서 `metadata_allocation_bound`는 profile 이벤트에서 `tracemalloc`의 최대 할당량을 읽어
그것이 올라간 것을 관측한 지점을 기록한다. 실패했을 때만 snapshot과 보고문을 만들고, 한계는 건드리지 않는다.
...
계측 자체는 공짜가 아니다. profile 함수를 걸면 CPython이 호출마다 frame 객체를 실제로 만든다.
frame 하나가 200 B 정도이므로 관측되는 최대치가 호출 깊이만큼 올라간다. 여기서는 8쌍 측정에서
1,535~1,852 B, 프로세스의 첫 창에서 12,436 B였고 실제 창의 여유는 약 1,034,000 B다.
다만 방향이 한쪽이라고 말할 수는 없다. 같은 할당이 GC를 앞당겨 원래라면 관측됐을 최대치를
낮추는 경우가 합성 workload에서 재현된다. 임의의 일시적 최대치를 창 안에서 지목하면서
측정값을 전혀 건드리지 않는 Python 설계는 없다. 별도의 진단 재실행은 측정값을 보존하지만
같은 순간의 할당을 지목하지는 못한다.

계측이 측정 대상을 붙잡는 문제는 따로 고쳤다. profile hook이 받은 C 메서드를 그대로 들고 있으면
그 수신자까지 살아남아, 재고 있는 버퍼 자체를 붙잡을 수 있었다. 지금은 짧은 문자열과 code 객체만
남긴다. 검토자의 재현에서 1,200,000 B 버퍼 두 개를 쓰는 workload의 최대치가 2,400,866 B였는데,
수정 후에는 no-op hook 대비 18~274 B 차이로 줄었다.
...
원인을 확정하지 못했으므로 이 결함은 닫지 않는다. 다음 CI 실패가 나면 최대 할당량이
올라간 것을 관측한 지점과 창 안에서 새로 import된 모듈이 실패 메시지에 그대로 남는다.
기록되는 것은 관측 지점이지 그 메모리를 소유한 호출의 증명이 아니다. 기준치보다 작은 상승은
기록하지 않으므로 마지막 기록은 보고된 최대치보다 최대 65,535 B까지 뒤처질 수 있다.
